...
//括号里写切点表达式，也就是对哪些类的哪些方法来进行拦截
    ...
    public void autoFill(JoinPoint joinPoint) {//前置通知这个方法往往需要它来传进来一个参数--->就是我们的连接点
        //通过这个连接点就可以知道当前哪个方法被拦截到了以及被拦截到的方法它具体的参数是什么样子，参数值、参数类型都可以获取到
        log.info("开始进行公共字段填充...");
